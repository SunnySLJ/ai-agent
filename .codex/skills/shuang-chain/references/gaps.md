...
- **护栏**：只写测试/编排配置、断言不弱化、禁伪造修复（含禁"偷偷 stub 回被测内部 feature""固定 sleep
  假装到点""起假服务冒充真栈"）、有界重试（链路 flake 不靠重跑/多睡掩盖）、产 PR 人审；发现真链路 bug
  **HALT 回交 TDD 修复流程/debugging**，不自行改产品码，**并提示回补下层**（见 SKILL.md 自愈护栏）。
- **归档**：每条新增回归挂 **journey 级可追溯 ID**（关联穿过的多 feature / 多 AC / 含哪些跳步）、
  按风险分级（**P0 关键旅程进发布门硬阻断**）、对齐三层节奏（链路 E2E 属最慢的 **L3**），遵循 shuang-blueprint。
```
