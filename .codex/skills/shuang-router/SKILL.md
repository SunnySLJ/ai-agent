...
name: shuang-router
description: >-
  中文：测试路由顾问；feature 全绿收尾时识别测试缺口类型，并路由到后端、前端、接缝或完整链路 skill。
  English: Routing advisor that classifies feature-completion testing gaps and routes them to backend, frontend, slice, or full-chain skills.
---
...
## 方法蓝本：统一遵循 `shuang-blueprint`

本 skill 的方法标准——**判类、缺口标注（✅已覆盖 / 🔧缺口）、三层节奏、可追溯、发布门**——
统一遵循 `shuang-blueprint` skill。本 skill 是该蓝本在「feature 收尾路由」这一环节
的落地：蓝本定义"什么是好的测试系统结构"，本 skill 负责"在收尾时判类并把缺口路由到对应执行器"。
任何与方法论相关的判断（怎么分层、缺口怎么标、发布门怎么设）以 `shuang-blueprint` 为准。
...
|---|---|---|
| **单后端** | → `shuang-backend` skill（按栈解析具体工具） | ✅ 执行器已建 |
| **单前端** | → `shuang-frontend` skill（按栈接成熟工具 + 配置 + 翻译视觉契约） | ✅ 执行器已建 |
| **局部前后端** | → `shuang-slice` skill（按栈起真栈 + 接缝断言 + 可选 diff-aware E2E） | ✅ 执行器已建 |
| **完整功能链路** | → `shuang-chain` skill（按栈挖通路 + P0 安全网 + 全系统编排 + 异步/时间/跨通道贯穿；gstack `/qa` 折叠进 UI 可走段，非依赖、无则回退） | ✅ 执行器已建 |
| **（可增类…）** | → 视该类性质而定 | 🔧 占位·待建 |

**单后端的具体工具不在本 skill 里写死**——交给 `shuang-backend` skill 按栈
（读 `package.json` / `pyproject.toml` / `go.mod` / `Cargo.toml` 等判栈后）解析。本 skill
只负责判出"这是单后端、命中了哪几类缺口"，并把这些缺口连同命中理由一并交给 `shuang-backend`。
...
- **单后端** → 路由到 `shuang-backend` skill。本 skill 只负责判出命中了哪几类后端能力缺口
  （真库 / 迁移、并发 / 限频原子性、韧性 / 降级、对象级越权 BOLA·BFLA 等），把缺口连同命中理由
  交给 `shuang-backend`，由它按栈解析具体工具。**本 skill 绝不写死 `pytest` / `pytest-postgresql`
  这类单栈答案。**
- **单前端** → 路由到 `shuang-frontend` skill。判类条件：改动**只落在前端层**（组件 / 页面 /
  ...
  把缺口连同命中理由交给 `shuang-frontend`，由它按栈解析具体工具。**形态差异：单后端执行器
  "自己写测试代码"，单前端执行器"接成熟工具（stylelint / Vitest+RTL / Playwright / axe / MSW）+
  配置 + 把项目视觉契约翻译成断言"，且唯一人审节点是 L2 视觉基线裁决。**
- **局部前后端** → 路由到 `shuang-slice` skill。判类条件：改动**同时落在前端和后端**、
  ...
  *真前端 ↔ 真后端* 单切片对账。本 skill 只负责判出命中了哪几类接缝缺口（① 环境编排：让两侧 + 依赖
  ...
  `shuang-slice`，由它按栈解析具体工具。**形态差异（一句话）：本格新难点在"起真栈
  （环境编排）"而非写断言；它是单前端 mock 与单后端真实的对账；第一层黑盒冒烟可选 diff-aware E2E
  （如 gstack `/qa`，非依赖、无则回退 Playwright），第二层再补结构化接缝断言。**
- **完整功能链路** → 路由到 `shuang-chain` skill。判类条件：这条被测路径**跨出了本 feature 边界、
  ...
  sleep；⑤ journey 级可追溯 + 安全网定位），把缺口连同命中理由交给 `shuang-chain`，由它按栈解析具体
  ...
  A→B→C 首次贯通"的落地入口——那条刚贯通的链路，就是 `shuang-chain` 的「通路挖掘」起点。
...
限频、韧性 / 降级在主流栈里通常都有现成测试库（✅可装，由 `shuang-backend` 按栈选具体包），唯独
...
运行器），所以 `shuang-frontend` 的**第一动作是立地基**。
...
所以 `shuang-slice` 的**第一动作是起真栈**（按栈起 docker-compose / 进程内组装等，gstack
...
不在任何单个 task 里，所以 `shuang-chain` 的**第一动作是从依赖图 / 跨模块契约 / user-story AC
...
2. **路由到哪个类别 skill** —— 据判定的类别，给出**路由去向**而非具体工具：
   - 单后端 → `shuang-backend` skill（具体工具由它按栈解析，本 skill 不写死）。
   - 单前端 → `shuang-frontend` skill（具体工具由它按栈接：stylelint / Vitest+RTL / Playwright /
     axe / MSW；唯一人审节点是 L2 视觉基线裁决，本 skill 不写死）。
   - 局部前后端 → `shuang-slice` skill（具体工具由它按栈起真栈 + 接缝断言；第一层
     黑盒冒烟可选 diff-aware E2E，如 gstack `/qa`，非依赖、无则回退 Playwright；本 skill 不写死）。
   - 完整功能链路 → `shuang-chain` skill（具体工具由它按栈挖通路 + 起全系统 + 编排非 UI 跳步；
...
3. **⭐ 是否补全了某条完整功能链路** —— 本 feature 是否补全了某条跨 feature 旅程 → 若是，
   列出该链路并建议端到端测试 + 路由去向（→ `shuang-chain`，它的「通路挖掘」就从这条
...
4. **逐类待补清单（带「覆盖状态」+「路由去向」列）** —— 各类别里哪些 TDD 已覆盖（仅核对）
   ...
   - `✅ 已被 TDD/Spec-Kit 覆盖（跳过）` —— TDD 循环 / spec-kit 流程已经覆盖，收尾时只核对、不补。
   - `🔧 结构性缺口（路由到对应类别 skill 闭环补测）` —— TDD / 流程没碰到的高风险接缝，
     需要在收尾闭环补测。**每个 🔧 项还要注明「路由去向」**（单后端 → `shuang-backend`；
     单前端 → `shuang-frontend`；局部前后端 → `shuang-slice`；完整功能链路 →
     `shuang-chain`；其余可增类 → 对应类别 skill / 占位待建）**和「现成 ✅ / 需自建 🔧」的栈无关
   ...
   - **条件命中（防过度测）**：单后端的缺口不全标，按本 feature 实际碰了什么筛子集——涉及
     ...
     接口才标 L6 契约 mock；而 L0/L1 测试地基常常为零，是 `shuang-frontend` 的第一动作。局部前后端
...
5. **状态标注**，给每条推荐都打上：
   - ✅ 已被 CI 闸 / TDD/Spec-Kit 流程自动覆盖 / 执行器 skill 已建
   - ⚠️ 需人工决策
...
- `references/backend-gaps.md` —— 单后端结构性缺口清单（栈无关能力）+ 每项「现成 ✅ / 需自建 🔧」判定（路由给 `shuang-backend` 执行器）。
- `references/frontend-gaps.md` —— 单前端结构性缺口清单（栈无关能力：地基 / 视觉回归 / a11y / 跨浏览器 + 响应式 / 契约 mock）+ 每项「接成熟工具 + 配置 + 翻译视觉契约」做法（路由给 `shuang-frontend` 执行器）。
- `references/fullstack-slice-gaps.md` —— 局部前后端结构性缺口清单（栈无关能力：环境编排 / 契约真实性 / 接缝粘合 / 真实时序·实时）+ 每项「起真栈 + 接缝断言」做法（路由给 `shuang-slice` 执行器）。
- `references/full-chain-gaps.md` —— 完整功能链路结构性缺口清单（栈无关能力：通路挖掘 / 关键性分级·选少 / 全系统编排·外部边界 stub / 异步·时间·跨通道贯穿 / journey 级可追溯·安全网定位）+ 每项「先挖通路再编排驱动」做法（路由给 `shuang-chain` 执行器）。
- `references/self-healing-guardrails.md` —— 任何自愈测试 agent 都必须遵守的 5 条护栏。

> 方法论统一以 `shuang-blueprint` skill 为蓝本；单后端缺口的实际补测由 `shuang-backend` skill 按栈执行，单前端缺口由 `shuang-frontend` skill 按栈执行，局部前后端缺口由 `shuang-slice` skill 按栈执行，完整功能链路缺口由 `shuang-chain` skill 按栈执行。
