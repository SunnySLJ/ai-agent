---
name: shuang-frontend
description: >-
  中文：前端闭环测试执行器；用于 feature 收尾时装配并固化视觉、交互、可访问性、契约与回归测试。
  English: Frontend closed-loop testing executor for installing and codifying visual, interaction, accessibility, contract, and regression checks after a feature is implemented.
---

# shuang-frontend · 单前端闭环测试执行器

## 这个 skill 解决什么

开发期 TDD和 spec/契约测试（spec-kit）覆盖的是"功能逻辑是否如规约工作"。
...
> **核心区别于 shuang-backend（务必读懂）**：后端那一类缺口（真库越权/并发/韧性）**没有 pip/npm
> 即装即用的工具**，shuang-backend 是"**施工队**"——大量测试代码要自己写。
...
遵循 `shuang-blueprint` 蓝本（按名引用即可）：风险分级排序、可追溯 ID、发布门、三层节奏。
...
**这是本 skill 与 shuang-backend 最大的流程差异**：前端项目的测试地基**常常为零**——
...
装上、接好配置、纳入 CI。**不自己造工具**（这是与 shuang-backend 的根本不同）。
...
- 按 `shuang-blueprint` 的**风险分级**排序（如越权式的视觉错误——涨跌色反了会误导决策——优先进发布门）。
...
- **发现真 bug 要 HALT，回交 TDD 修复流程 修**——本 skill **不自行改产品代码**。补测过程中若 RED
  暴露的是产品真实缺陷（不只是缺覆盖），**停下来**，把缺陷回交 TDD 纪律 /
  systematic debugging 纪律 走修复闭环，修完再回到本 skill 把回归固化。
...
- 上游：`shuang-router` 判定"单前端"时调用本 skill（也可被用户直接触发）。
- 蓝本：所有归档/分级/发布门/节奏遵循 `shuang-blueprint`（按名引用，不在此复制其内容）。
- 兄弟：`shuang-backend` 处理"单后端"；二者根本分工不同——后端**自建测试代码**（施工队），
  前端**装配成熟工具 + 翻译视觉契约**（装配工 + 监理）。
- 方法论复用：发现真缺陷的修复与调试复用 TDD 纪律 和
