...
- **护栏**：只写测试/规则、断言不弱化、禁伪造修复（含禁"自动刷新基线掩盖回归"）、有界重试、产 PR 人审；
  发现真 bug **HALT 回交 TDD 修复流程**，不自行改产品码（见 SKILL.md 自愈护栏）。
- **归档**：每个新增规则/回归挂可追溯 ID、按风险分级、纳入发布门、对齐三层节奏（遵循 shuang-blueprint）。
