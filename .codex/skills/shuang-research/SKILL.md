---
name: shuang-research
description: >-
  中文：0 到 1 产品立项调研流程；通过问答抽取项目骨架，生成四路并行调研与收敛提示。
  English: 0-to-1 product research kickoff workflow that extracts project structure through Q&A and generates parallel research plus synthesis prompts.
---
...
把"新产品立项前期调研"标准化为一套**可复用的元方法论**：4 个独立调研主题并行 + 双搜索引擎交叉验证 + 1 个汇总收敛。输入是用户的一个粗想法，输出是 5 份调研文档 + 驱动 Codex 完成它们的并行 Task 提示词。

它在 Vibe Coding 工作流里的位置（5 段链路）：

```
想法 → [本 Skill：立项调研]
       → ★ 对抗调研（shuang-arch，多候选时强烈建议）
       → Brainstorming（方向收敛）
       → PRD（shuang-prd）
...
方案中做选型时，**强烈建议**在 shuang-brainstorm 之前先用
`shuang-arch` Skill 做对抗调研，产出架构基线决策。

**它不是什么**：不是 PRD 写作器（用 `shuang-prd`）、不是技术方案设计器（那是 spec-kit `/plan` 阶段）、不是 shuang-brainstorm（shuang-brainstorm 发散收敛，本 Skill 事实摸底）。

**为什么中间必须插 shuang-brainstorm**：调研只解决"市面上能做什么、有什么资源"，**不解决"我到底要做哪个版本"**。从调研直接跳 PRD = 在所有可能性里抓阄。shuang-brainstorm 用苏格拉底追问把方向收敛到一个明确的 MVP，PRD 才能写得不发散。
...
每个 sub-agent 必须同时用：
- **路 A**：Codex 内置 `WebSearch` / `WebFetch`（偏海外/英文/官方/GitHub）
...
不适用：
- 想法太空（"我想做个 app"）→ 先做 shuang-brainstorm 发散
- 已经有调研产物 → 进 shuang-brainstorm（不要直接跳 PRD）
- 是已有产品的小迭代 → 用 shuang-brainstorm
...
- [ ] 衔接了完整三段链路（shuang-brainstorm → PRD → Spec-Kit），不是直接跳 PRD？
- [ ] 三段触发提示词都给了，用户复制即可推进？

📖 **何时读 [references/handoff-to-prd.md](references/handoff-to-prd.md)**：当用户问"调研完之后怎么走"、"PRD 要写啥"、"PRD 怎么转 spec-kit"时读取。这是完整的下游链路方法论（4 段链路 / 11 问 shuang-brainstorm 议程 / PRD 12 章节 / PRD→Spec-Kit 7 规则）。
...
之间纠结，**先**用 `shuang-arch` Skill 做对抗调研：

> 我有多个候选技术方案：[列出候选]
> 请用 shuang-arch Skill 做架构基线决策。
...
### ① 用 shuang-brainstorm Skill 收敛 MVP 方向
...
> shuang-brainstorm Skill 帮我收敛 MVP 方向。
>
> 📂 必读文件（按优先级排序）：
...
>   已通过 5 角色法庭辩论决定，shuang-brainstorm 不重开此案。
...
> 请仔细阅读 specs/research/ 下的 5 份调研，然后用 shuang-brainstorm
...
### ② 用 shuang-prd Skill 写 PRD
触发提示词（极简版，规范在 shuang-prd Skill 内部）：
> 基于 shuang-brainstorm 的收敛结果和 specs/research/ 下的调研文件，
> 用 shuang-prd Skill 帮我写 PRD（产出到 specs/prd.md）。
>
> ⚠️ 如果 specs/research/06-架构基线决策.md 存在：
> PRD 需新增"架构基线"小节，搬运 06 的复用矩阵。

💡 设计原则：shuang-prd Skill 已编码 12 章节规范、AC 格式、NFR 数字化、
...
- 跳过 shuang-brainstorm 直接写 PRD → PRD 必然发散
...
- ❌ 不要在主 Codex session 里直接做调研。**必须派 4 个 Task**——保护主 session context、真并行省时间
...
2. 用户复制粘贴这份提示词到 Codex 即可启动 4-agent 并行调研
