...
> 本文是 `shuang-research` Skill 的下游衔接规范，给"调研完之后到底怎么一路走到 Spec-Kit"
...
- [一、4 段链路总览](#一4-段链路总览)
- [二、Brainstorming 必须确定的 11 个问题](#二shuang-brainstorm-必须确定的-11-个问题)
...
**对抗调研何时跳过**：
- 03-开源项目.md 推荐 0 或 1 个候选 → 跳过，直接 shuang-brainstorm
- 已经做完技术选型，没有架构分歧 → 跳过
...
调研产物落地后，shuang-brainstorm 阶段必须帮用户回答完以下 11 个问题——**这 11 个答案就是 PRD 的输入清单**。如果某个问题答不上来，说明 shuang-brainstorm 没收敛到位，不要急着写 PRD。
...
**shuang-brainstorm 禁止重开**：架构基线（用什么 fork）、技术选型（用什么框架）、06 中已被否决的方案——这些是 06 的成果，不重新讨论。
...
| # | 章节 | 必备 | 一句话目的 | 对应 shuang-brainstorm 问题 |
|---|------|:---:|----------|----------------------|
...
### PRD 触发提示词（极简版，规范在 shuang-prd Skill 内部）
...
⚠️ 唯一需要额外提示的是"06 架构基线小节"——这是 shuang-arch
Skill 之后才出现的新场景，shuang-prd 初版可能未编码。
...
| # | 反模式 | 后果 | 处方 |
|---|--------|-----|-----|
| 1 | 跳过 shuang-brainstorm 直接写 PRD | PRD 必然发散（调研只解决"能做什么"，不解决"该做哪个"）| 强制 shuang-brainstorm 收敛 11 问 |
