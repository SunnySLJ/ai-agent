---
name: shuang-blueprint
description: >-
  中文：测试体系蓝本；用于制定风险分级、需求追溯、三层测试节奏、闭环补测和发布门策略。
  English: Testing-system blueprint for defining risk tiers, traceability, layered testing cadence, closed-loop backfill, and release gate strategy.
---
...
**谁用它**：`shuang-router`（决定一个任务该走哪类测试）与各类别测试 skill
（如 `shuang-backend`、`shuang-frontend`）。它们把本蓝本的抽象原语，落到具体栈与具体项目上。
...
表里故意不写死库名——具体到哪个库，由 `shuang-router` 或类别 skill 读项目栈后实例化。

---

## 怎么用本蓝本（给遵循它的 skill）

1. **路由器（shuang-router）**：用 §一 给任务定级、用 §三 决定落到哪一层、用 §六 把能力交给对应栈的类别 skill。
2. **类别 skill（如 shuang-backend）**：在自己负责的栈里，把 §六 的能力实例化为具体工具，
