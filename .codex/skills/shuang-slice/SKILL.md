...
name: shuang-slice
description: >-
  中文：局部前后端接缝测试执行器；用于单 feature 内停止 mock，对齐前端消费者假设与后端真实行为。
  English: Full-stack slice testing executor for replacing mocks within one feature and reconciling frontend consumer assumptions with real backend behavior.
---

# shuang-slice · 局部前后端接缝测试执行器

## 这个 skill 解决什么

`shuang-backend` 把前端 mock 掉、单独验后端的真实；`shuang-frontend` 把后端 mock 掉、单独验前端的渲染与交互契约。
...
3. **GREEN**：让断言转绿。
   - 若红暴露的是**真实接缝 bug**（典型：前端 mock 撒的谎在这里穿帮了）——**HALT，不要自己改产品码**
     （见自愈护栏），回交 TDD 纪律 / systematic debugging 纪律 修，
     修完再回本 skill 把回归固化。
...
- 按 `shuang-blueprint` 的**风险分级**排序进发布门——**越权 / 契约漂移属高风险**（前者放行别人数据、
...
前两格（shuang-backend / shuang-frontend）的共同前提是：**可以把另一侧 mock 掉**，于是各自能独立、廉价地跑。
...
- **发现真 bug 要 HALT，回交 TDD/debugging 修复流程**——本 skill **不自行改产品代码**。补测中若 RED 暴露的是
  真实接缝缺陷（前端假设与后端真实对不上、凭证没透传、错误没映射、时序错乱等），**停下来**，
  把缺陷回交 TDD 纪律 / systematic debugging 纪律 走修复闭环，
  修完再回本 skill 把回归固化。
