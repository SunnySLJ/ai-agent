---
name: shuang-design-inject
description: >-
  中文：Spec-Kit 设计系统注入流程；用于已有 spec/plan/tasks 后补 DESIGN.md、Figma、Stitch 或原型资料，不重跑完整管线。
  English: Spec-Kit design-system injection workflow for adding DESIGN.md, Figma, Stitch, or prototype guidance to existing spec/plan/tasks without rerunning the full pipeline.
---
...
2. **Then** you produce a design system (via Google Stitch, Figma, Codex Design, or hand-writing DESIGN.md)
...
| Design artifact in hand | One of: DESIGN.md file / Stitch download zip / Figma export / Codex Design output / HTML prototypes |
...
├── design-reference/
│   └── <source>-export/                   ← e.g. stitch-export/, figma-export/, Codex-design-export/
│       ├── <page-1>/
...
**Source adapter logic** — the actual move differs by source. Read `references/design-source-adapters.md` for the exact mapping rules for each source type (Stitch / Figma / Codex Design / hand-written / other).
...
- `references/design-source-adapters.md` — how to map Stitch / Figma / Codex Design / hand-written outputs into the standard project structure (Step 1 details)
